#!/usr/bin/env python3
"""Fix AurelEconomy.java - remove duplicate code from the 26.x branch."""
...
# Fix duplicate init block - the second block has slightly different comments
# Find the second occurrence of the init block and remove it
init_block_2 = """ // Initialize custom item system
...
if content.count(init_block_2) >= 1:
    # Remove the second occurrence (the one with "1 second" comment)
    content = content.replace(init_block_2, "", 1)
...
# Fix duplicate customitems command registration
...
# Also fix duplicate in onDisable
content = content.replace(
...
result = push_file("src/main/java/com/aureleconomy/AurelEconomy.java", content,
                    "fix: remove duplicate code in AurelEconomy.java", BRANCH, token, sha)
