...
url = f"https://api.github.com/repos/{REPO}/actions/runs/26312203081/jobs"
req = urllib.request.Request(url, headers=headers)
with urllib.request.urlopen(req) as resp:
    data = json.loads(resp.read())

for job in data["jobs"]:
    if job["name"] == "custom-item-detection-test" and job.get("conclusion") == "failure":
        ...
        # Print all non-trivial lines
        for line in logs.split("\n"):
            c = line.strip()
            if len(c) > 28 and c[4:5] == '-' and c[10:11] == 'T':
                c = c[28:].strip()
            if not c: continue
            if c.startswith('+') or c.startswith('##') or c.startswith('2026'): continue
            # Skip super verbose docker/gcloud lines
            if 'docker' in c.lower() and ('pull' in c.lower() or 'image' in c.lower()): continue
            if 'node_modules' in c: continue
            print(c[:300])
