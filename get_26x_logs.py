#!/usr/bin/env python3
"""Get specific build log for 26.x branch latest run."""
...
# Get latest run for 26.x branch
...
if not build_job:
    print("No build job found")
    exit()

job_id = build_job["id"]

# Get logs via redirect
conn = http.client.HTTPSConnection("api.github.com")
conn.request("GET", f"/repos/{repo}/actions/jobs/{job_id}/logs", headers=headers)
...
if resp.status in (301, 302, 303, 307):
    ...
    for line in logs.split("\n"):
        ...
        if any(kw in lower for kw in ["error:", "cannot find symbol", "does not exist",
                                       ...
                                       "cannot access", "class file has wrong version"]):
            print(f"  {clean[:400]}")
    print()
else:
    body = resp.read().decode("utf-8", errors="replace")
    print(f"Status: {resp.status}, body: {body[:300]}")
    conn.close()
