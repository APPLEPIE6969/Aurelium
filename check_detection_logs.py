...
url = f"https://api.github.com/repos/{REPO}/actions/runs/26309769899/jobs"
    ...
    if job["name"] == "custom-item-detection-test" and job.get("conclusion") == "failure":
        ...
        for line in logs.split("\n"):
            ...
            if len(c) > 28 and c[4:5] == '-' and c[10:11] == 'T':
                c = c[28:].strip()
            if not c: continue
            if any(kw in c.lower() for kw in ["executableitems", "score", "aurelium", "scanner", "fail", "error", "exception", "loaded", "started", "done", "warn", "paper"]):
                print(c[:300])
