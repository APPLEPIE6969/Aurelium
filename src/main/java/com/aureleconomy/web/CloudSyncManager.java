    ...
    private void attemptRegistration(int attempt) {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                if (msg != null && (msg.contains("HTTP 403") || msg.contains("HTTP 401"))) {
                    registrationFailureCount++;
                    // Auto-recover from stale entry: dashboard now auto-deletes stale entries, so retry with SAME serverId
                    if (msg.contains("stale entry") || msg.contains("already registered with different API key")) {
                        plugin.getComponentLogger().warn("Stale dashboard entry detected — retrying with same serverId (dashboard auto-cleans stale entries)...");
                        if (attempt < 3) {
