...
public class CustomItemsCommand implements TabExecutor {
 ...
 // Fix: clamp page to valid range [1, totalPages] to prevent negative index crash
 ...
 // Replace in registry using upsert (fix: was register, should be upsert)
 ...
 // Fix: validate prices are finite and non-negative
 if (Double.isNaN(buy) || Double.isInfinite(buy) || buy < 0) {
 sender.sendMessage(Component.text("Buy price must be a non-negative finite number.", NamedTextColor.RED));
 return;
 }
 if (Double.isNaN(sell) || Double.isInfinite(sell) || sell < 0) {
 sender.sendMessage(Component.text("Sell price must be a non-negative finite number.", NamedTextColor.RED));
 return;
 }
