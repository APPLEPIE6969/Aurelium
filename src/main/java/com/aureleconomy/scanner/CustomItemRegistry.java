 ...
 */
 ...
 synchronized (this) {
 // Fix: double-check ALL dedup keys under lock, not just canonicalId
 // This prevents two concurrent registrations with different IDs but same
 // PDC/model/lore/native/hash from both passing pre-checks and getting inserted
 ...
 // Re-check item hash under lock too
 String lockedItemHash = itemHash; // already computed above
 if (itemHashToId.containsKey(lockedItemHash)) {
 String existingId = itemHashToId.get(lockedItemHash);
 CustomMarketItem existingItem = itemsById.get(existingId);
 if (existingItem != null && existingItem.getItemStack().isSimilar(item.getItemStack())) {
 addDiscoveryMethod(existingId, method);
 duplicatesPrevented.incrementAndGet();
 ...
 itemHashToId.put(lockedItemHash, canonicalId);
 addDiscoveryMethod(canonicalId, method);
 }
