 ...
 *
 * Note: CustomItemRegistry(AurelEconomy) requires a plugin instance for
 * the register() method (it calls plugin.getConfig() and plugin.getMarketManager()).
 * We use the no-arg constructor path where possible, or test dedup logic
 * that doesn't reach the market-adding code path.
 *
 * Since the real constructor requires AurelEconomy, these tests focus on
 * the dedup map logic which can be verified through the public query methods
 * after registration. The register() method's market-add branch will NPE
 * without a real plugin, so we test via a subclass or by catching the NPE
 * after the dedup logic has already run.
 */
class CustomItemRegistryTest {

    /**
     * Create a registry with a null plugin. The register() method will NPE
     * when trying to add to market, but the dedup logic runs before that.
     * We catch the NPE and verify the dedup maps were updated correctly.
     */
